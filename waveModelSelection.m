function [waveRange, waveType] = waveModelSelection(varargin)
...
% hoverLMax = 1;
% aoverLMax = 0.07;
waveType = cell(length(h),1); waveRange = cell(length(h),1);
...
waveRange(or(in1,on1)) = {'A'}; %1;
waveType(or(in1,on1)) = {'Linear'}; %1;
...
waveRange(or(in2,on2)) = {'B'}; %1;
waveType(or(in2,on2)) = {'2nd order'}; %2;
...
waveRange(or(in3,on3)) = {'C'}; %1;
waveType(or(in3,on3)) = {'3rd order'}; %3;
...
waveRange(or(in5,on5)) = {'D'}; %1;
waveType(or(in5,on5)) = {'5th order'}; %5;
...
waveRange(or(inLWDSWW,onLWDSWW)) = {'F'}; %1;
waveType(or(inLWDSWW,onLWDSWW)) = {'F. linear weakly dispersive shallow water wave'}; %7;
...
waveRange(or(inBoussi,onBoussi)) = {'G'}; %1;
waveType(or(inBoussi,onBoussi)) = {'G. classic Boussinesq approximation'}; %7;
...
waveRange(or(inLnonDSWW,onLnonDSWW)) = {'H'}; %1;
waveType(or(inLnonDSWW,onLnonDSWW)) = {'H. linear non dispersive shallow water wave'}; %7;
...
waveRange(or(inNLSWW,onNLSWW)) = {'I'}; %1;
waveType(or(inNLSWW,onNLSWW)) = {'I. non linear shallow water wave'}; %7;
...
waveType(or(inBreak,onBreak)) = {'J. Breaking'}; %8;
